...
// Metadata remains decoupled from presentation logic.
const sequenceMetadata = [
  {
    title: 'Model 1: Pure & Slotted ALOHA (`model1_aloha.py`)',
    scriptPath: 'src/scripts/ethernet-sequence/model1_aloha.py',
  ...
  },
  {
    ...
    scriptPath: 'src/scripts/ethernet-sequence/model5_acknowledged_protocol.py',
  ...
  },
  {
    ...
    scriptPath: 'src/scripts/ethernet-sequence/model6_full_network.py',
...
// This Server Component's sole responsibility is to assemble the data for our computational proofs.
// It reads the script files from the filesystem, guaranteeing their presence at build time.
const EthernetSequenceDisplay = async () => {
  const sequenceData = await Promise.all(
    sequenceMetadata.map(async (meta) => {
      const filePath = path.join(process.cwd(), meta.scriptPath);
      try {
