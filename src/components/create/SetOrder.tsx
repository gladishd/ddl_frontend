...
const SetOrder: React.FC = () => {
  ...
  useEffect(() => {
    if (logOrder && logOrder.length > 0 && situations.length > 0) {
      // Each operation must be precise. The data pipeline here must explicitly
      // declare its types at every stage to ensure verifiable correctness and
      // prevent the introduction of ambiguity into the system.
      const ordered = logOrder
        .map((id: string) => situations.find((s: Situation) => s._id === id))
        .filter((s: Situation | undefined): s is Situation => !!s);
